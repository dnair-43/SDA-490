...
# ── logged_wealth: pull from wb_data_clean if available, else WDI iso2 lookup ──
if (exists("wb_data_clean") && is.data.frame(wb_data_clean) &&
    "gni_pc" %in% names(wb_data_clean)) {
  # Harmonise country names from iso3c to display names used in wvs_countries_list
  if (!"country" %in% names(wb_data_clean)) {
    library(countrycode)
    wb_gni_scatter <- wb_data_clean %>%
      mutate(country = countrycode(iso3c, "iso3c", "country.name")) %>%
      filter(!is.na(gni_pc), !is.na(country)) %>%
      transmute(country, year, logged_wealth = log(gni_pc))
  } else {
    wb_gni_scatter <- wb_data_clean %>%
      filter(!is.na(gni_pc)) %>%
      transmute(country, year, logged_wealth = log(gni_pc))
  ...
  wb_gni_scatter <- data.frame(country = character(), year = integer(),
                                logged_wealth = numeric())
}
...
ui <- navbarPage(
  ...
  tabPanel("Overview",
           ...
           and is negatively affecting everyone."
                   ...
                   # Timeline chart — place Timeline.png in the app's www/ folder
                   tags$div(
