...
if (file.exists(wvs_path)) {
  ...
  set.seed(42)
  wvs_countries <- c("USA", "Germany", "China", "India", "Brazil",
...
wvs_countries_list <- sort(unique(as.character(wvs_data$country)))
  ...
  tags$head(
    ...
    tags$style(HTML("
      ...
      * { box-sizing: border-box; }
      ...
      /* Override slider colours for WVS tab to gold */
      .wvs-sidebar .irs--shiny .irs-bar { background: #c9a96e !important; border-color: #c9a96e !important; }
      .wvs-sidebar .irs--shiny .irs-handle { background: #c9a96e !important; border-color: #c9a96e !important; }
      .wvs-sidebar .irs--shiny .irs-from,
      .wvs-sidebar .irs--shiny .irs-to,
      .wvs-sidebar .irs--shiny .irs-single { background: #c9a96e !important; color: #0f1117 !important; }
      /* Selectize in WVS */
      .wvs-sidebar .selectize-control .selectize-input {
        background: #1a1d27 !important; border: 1px solid #2a2d3a !important;
        color: #e8e6df !important; border-radius: 6px !important;
        font-size: 0.82rem !important; box-shadow: none !important; padding: 7px 10px !important;
      }
      .wvs-sidebar .selectize-dropdown {
        background: #1e2130 !important; border: 1px solid #2a2d3a !important;
        color: #e8e6df !important; border-radius: 6px !important;
      }
      .wvs-sidebar .selectize-dropdown .option:hover,
      .wvs-sidebar .selectize-dropdown .option.active { background: #2a2d3a !important; color: #c9a96e !important; }
      .wvs-sidebar .item { color: #c9a96e !important; }
    "))
  ),
...
server <- function(input, output, session) {
  ...
  wvs_filtered <- reactive({
    ...
    .wvs_data %>%
      filter(
        country %in% input$wvs_countries,
  ...
  output$wvs_scatter <- renderPlot({
    ...
    if (isTRUE(input$color_country)) {
      ...
      p <- p +
        ...
        scale_color_manual(values = cpal, name = "Country")
    } else {
      p <- p +
        geom_line(aes(group = country), color = "#c9a96e", alpha = .3, linewidth = .7) +
