  ...
  div(class = "main-wrap",
      ...
      div(class = "side-panel",
          ...
          div(class = "source-note",
              "Source: Varieties of Democracy (V-Dem) Dataset v15.", tags$br(),
              "v2x_polyarchy = Electoral Democracy Index.", tags$br(),
              "Coppedge et al. (2024). SSRN 4000971."
          )
      ),
...
server <- function(input, output, session) {
  ...
  # Join geometry to the selected year's V-Dem data
  year_data <- reactive({
    yr <- input$year
    vdem_yr <- vdem_all |> filter(year == yr) |> select(country_name, v2x_polyarchy)
    world_base |> left_join(vdem_yr, by = c("vdem_name" = "country_name"))
  })
  
  filtered <- reactive({
    year_data() |>
      mutate(visible = !is.na(v2x_polyarchy) &
               v2x_polyarchy >= input$range[1] &
               v2x_polyarchy <= input$range[2])
  })
  
  output$header_year <- renderText({
    paste0("V-Dem · Electoral Democracy · ", input$year)
  })
  ...
  observeEvent(input$map_shape_mouseover, {
    ...
    clicked_name <- hov$id
    row <- filtered() |> filter(name_long == clicked_name)
  ...
  output$n_countries <- renderText({
    filtered() |> filter(visible & !is.na(v2x_polyarchy)) |> nrow()
  })
  output$avg_score <- renderText({
    vals <- filtered() |> filter(visible & !is.na(v2x_polyarchy)) |> pull(v2x_polyarchy)
